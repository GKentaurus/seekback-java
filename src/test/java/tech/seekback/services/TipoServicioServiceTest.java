...
import static faker.FakerData.tipoServicioFaker;
...
class TipoServicioServiceTest {
  ...
  public void setUp() {
    ...
    obj = tipoServicioFaker();
  }

  @Test
  public void createOne() {
    TipoServicio faker = tipoServicioFaker();
    TipoServicio persisted = null;
    Exception exception = null;

    try {
      when(dao.create(any(TipoServicio.class))).thenReturn(faker);
  ...
  public void createAll() {
    List<TipoServicio> fakerList = new ArrayList<>();
    Exception exception = null;
    for (int i = 0; i < 4; i++) {
      fakerList.add(tipoServicioFaker());
    }
    try {
  ...
  public void createAllThrowsException() {
    TipoServicio fakerList = tipoServicioFaker();
    Exception exception = null;
    try {
      when(dao.checkIfExist(any(), anyString(), any())).thenReturn(true);
      service.create(fakerList);
  ...
  public void getOne() throws ConnectionExcep {
    ...
    TipoServicio persisted = service.getOne(1);
    assertNotNull(persisted);
  }
